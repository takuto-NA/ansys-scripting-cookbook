...
## 5. STEP インポートの注意点と詳細 (2024 R2)

STEP 形式を使用する際の詳細な仕様と注意点です。

- **単位の自動認識**: ファイル内の長さ単位（mm, m 等）は自動的に読み取られ、Mechanical 側に引き継がれます。
- **名前の重複**: アセンブリレベルのパーツとルートレベルのパーツで同名が存在する場合、インポートに失敗することがあります。その場合は SpaceClaim で一度開いて保存し直すことが推奨されます。
- **閉じた面/中空ボディ**: STEP 内の閉じた面などは、Mechanical ではフルソリッドとして転送されます。
- **DesignModeler の制限**: 
    - 500m（または 1km^3）を超える巨大なモデルはインポートできません。
    - 色情報の NS 変換には対応していません（SpaceClaim または Reader モードを推奨）。
